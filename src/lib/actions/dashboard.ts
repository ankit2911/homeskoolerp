...
export async function getTodaySessionStats(): Promise<TodayStats> {
    ...
    const calendarEntries = await db.academicCalendarEntry.findMany({
        where: {
            OR: [
...
export async function getOperationalFlags(): Promise<OperationalFlag[]> {
    ...
    // 1. Vacant sessions (no teacher assigned) - Critical
    const vacantSessions = await db.session.findMany({
        where: {
            teacherId: null,
    ...
    const calendarEntries = await db.academicCalendarEntry.findMany({
        where: {
            ...
            type: { in: ['HOLIDAY', 'EXAM_DAY'] }
        }
    });

    if (calendarEntries.length > 0) {
        // Check if there are scheduled sessions on this calendar day
        const scheduledOnConflict = await db.session.count({
            where: {
                status: 'SCHEDULED',
                startTime: { gte: todayStart, lte: todayEnd }
            }
    ...
    // 3. Unlogged sessions (past sessions pending log) - Info
    const unloggedSessions = await db.session.findMany({
        where: {
            status: 'PENDING_LOG',
            endTime: { lt: new Date() }
        },
...
export async function getRecentActivities(limit = 10): Promise<ActivityItem[]> {
    ...
    // Recent sessions (created or starting today/yesterday)
    ...
    const recentCalendar = await db.academicCalendarEntry.findMany({
        where: {
...
export async function getScheduleData(filters: {
    ...
    // Check calendar conflicts for the date range
    const calendarEntries = await db.academicCalendarEntry.findMany({
        where: {
            OR: [
